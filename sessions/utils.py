...
def envelope_max_mean(t: np.ndarray, signal: np.ndarray, env_distance_s: float = 15.0, min_height_frac: float = 0.3) -> tuple[float, np.ndarray, np.ndarray]:
    t = np.asarray(t)
    signal = np.asarray(signal)
    height_threshold = min_height_frac * float(np.max(np.abs(signal)))
    ...
    def _envelope_indices(s: np.ndarray) -> np.ndarray:
        ...
        env_idx_in_swing, _ = find_peaks(s[swing_idx], distance=distance_samples, height=height_threshold)
